...
public:
	...
	BOOL Create(CWnd *pWndParent,LPCTSTR szText[2]=NULL,DWORD dwTimeout=2000,DWORD dwStyle=CSS_FADE | CSS_CENTERSCREEN | CSS_SHADOW);
	...
	void SetText0(LPCTSTR szText);
	void SetText1(LPCTSTR szText);
	void SetTextFont(LPCTSTR szFont,int nSize,int nStyle);
	void SetTextDefaultFont();
	void SetTextColor0(COLORREF crTextColor);
	void SetTextColor1(COLORREF crTextColor);
	void SetTextRect0(CRect& rcText);
	void SetTextRect1(CRect& rcText);
